...
export default function University() {
  ...
  const [uniTaskSubject, setUniTaskSubject] = useState("")
  const [uniTaskTitle, setUniTaskTitle] = useState("")
  const [uniTaskDue, setUniTaskDue] = useState("")
  const [uniProjectSubject, setUniProjectSubject] = useState("")
  const [uniProjectName, setUniProjectName] = useState("")
  const [uniProjectDescription, setUniProjectDescription] = useState("")
  const [uniProjectDeadline, setUniProjectDeadline] = useState("")
  ...
  const addAttendance = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/university/attendance", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ subject_id: Number(attendanceSubject), class_date: attendanceDate, attended: attendancePresent }) })
      if (!response.ok) throw new Error("Nu am putut înregistra prezența")
      return response.json()
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["university"] }); setShowAdd(false); setAttendanceSubject(""); setAttendanceDate("") },
  })
  const addUniTask = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: uniTaskTitle, due_date: uniTaskDue || null, university_subject_id: Number(uniTaskSubject) }) })
      if (!response.ok) throw new Error("Nu am putut adăuga taskul")
      return response.json()
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["university"] }); setShowAdd(false); setUniTaskSubject(""); setUniTaskTitle(""); setUniTaskDue("") },
  })
  const addUniProject = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: uniProjectName, description: uniProjectDescription || null, deadline: uniProjectDeadline || null, university_subject_id: Number(uniProjectSubject) }) })
      if (!response.ok) throw new Error("Nu am putut adăuga proiectul")
      return response.json()
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["university"] }); setShowAdd(false); setUniProjectSubject(""); setUniProjectName(""); setUniProjectDescription(""); setUniProjectDeadline("") },
  })
    ...
    <div className="card-liquid-page">
      <div className="card-liquid-page-content p-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="mb-6 flex items-center justify-between"><label className="relative flex w-fit items-center gap-2 text-sm font-semibold text-text-primary"><span>University · {view}</span><ChevronDown className="w-4 h-4 text-text-muted" /><select value={view} onChange={(e) => setView(e.target.value)} className="absolute inset-0 cursor-pointer opacity-0" aria-label="Alege secțiunea University">{["Overview", "Materii", "Note", "Prezențe", "Examene", "Taskuri", "Proiecte", "Orar"].map((item) => <option key={item}>{item}</option>)}</select></label><button type="button" onClick={() => setShowAdd((v) => !v)} className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-white" aria-label="Adaugă în University"><Plus className="w-4 h-4" /></button></div>
          {showAdd && <div className="mb-6 rounded-2xl border border-border bg-surface/70 p-4"><h2 className="mb-3 text-sm font-semibold">{view === "Proiecte" ? "Adaugă proiect universitar" : view === "Taskuri" ? "Adaugă task universitar" : view === "Note" ? "Adaugă notă" : view === "Examene" ? "Adaugă examen" : view === "Prezențe" ? "Înregistrează prezență" : "Adaugă materie"}</h2>{view === "Proiecte" ? <><div className="grid gap-2 sm:grid-cols-2"><select value={uniProjectSubject} onChange={(e) => setUniProjectSubject(e.target.value)} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="">Alege materia</option>{subjects.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select><input value={uniProjectName} onChange={(e) => setUniProjectName(e.target.value)} placeholder="Nume proiect" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><input value={uniProjectDescription} onChange={(e) => setUniProjectDescription(e.target.value)} placeholder="Descriere" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><input value={uniProjectDeadline} onChange={(e) => setUniProjectDeadline(e.target.value)} type="date" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /></div><div className="mt-3 flex gap-2"><button type="button" onClick={() => setShowAdd(false)} className="rounded-xl border border-border px-3 py-2 text-xs text-text-secondary">Anulează</button><button type="button" onClick={() => addUniProject.mutate()} disabled={!uniProjectSubject || !uniProjectName.trim() || addUniProject.isPending} className="rounded-xl bg-primary px-3 py-2 text-xs text-white">Adaugă proiect</button></div></> : view === "Taskuri" ? <><div className="grid gap-2 sm:grid-cols-3"><select value={uniTaskSubject} onChange={(e) => setUniTaskSubject(e.target.value)} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="">Alege materia</option>{subjects.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select><input value={uniTaskTitle} onChange={(e) => setUniTaskTitle(e.target.value)} placeholder="Task universitar" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><input value={uniTaskDue} onChange={(e) => setUniTaskDue(e.target.value)} type="date" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /></div><div className="mt-3 flex gap-2"><button type="button" onClick={() => setShowAdd(false)} className="rounded-xl border border-border px-3 py-2 text-xs text-text-secondary">Anulează</button><button type="button" onClick={() => addUniTask.mutate()} disabled={!uniTaskSubject || !uniTaskTitle.trim() || addUniTask.isPending} className="rounded-xl bg-primary px-3 py-2 text-xs text-white">Adaugă task</button></div></> : view === "Prezențe" ? <><div className="grid gap-2 sm:grid-cols-3"><select value={attendanceSubject} onChange={(e) => setAttendanceSubject(e.target.value)} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="">Alege materia</option>{subjects.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select><input value={attendanceDate} onChange={(e) => setAttendanceDate(e.target.value)} type="date" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><select value={attendancePresent ? "present" : "absent"} onChange={(e) => setAttendancePresent(e.target.value === "present")} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="present">Prezent</option><option value="absent">Absent</option></select></div><div className="mt-3 flex gap-2"><button type="button" onClick={() => setShowAdd(false)} className="rounded-xl border border-border px-3 py-2 text-xs text-text-secondary">Anulează</button><button type="button" onClick={() => addAttendance.mutate()} disabled={!attendanceSubject || !attendanceDate || addAttendance.isPending} className="rounded-xl bg-primary px-3 py-2 text-xs text-white">Salvează</button></div></> : view === "Examene" ? <><div className="grid gap-2 sm:grid-cols-4"><select value={examSubject} onChange={(e) => setExamSubject(e.target.value)} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="">Alege materia</option>{subjects.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select><input value={examDate} onChange={(e) => setExamDate(e.target.value)} type="date" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><select value={examType} onChange={(e) => setExamType(e.target.value)} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="final">Examen</option><option value="partial">Test</option><option value="colocviu">Colocviu</option><option value="restanta">Restanță</option></select><input value={examRoom} onChange={(e) => setExamRoom(e.target.value)} placeholder="Sală" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /></div><div className="mt-3 flex gap-2"><button type="button" onClick={() => setShowAdd(false)} className="rounded-xl border border-border px-3 py-2 text-xs text-text-secondary">Anulează</button><button type="button" onClick={() => addExam.mutate()} disabled={!examSubject || !examDate || addExam.isPending} className="rounded-xl bg-primary px-3 py-2 text-xs text-white">Adaugă examen</button></div></> : view === "Note" ? <><div className="grid gap-2 sm:grid-cols-4"><select value={gradeSubject} onChange={(e) => setGradeSubject(e.target.value)} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="">Alege materia</option>{subjects.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}</select><input value={gradeValue} onChange={(e) => setGradeValue(e.target.value)} placeholder="Nota" type="number" min="1" max="10" step="0.01" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><select value={gradeType} onChange={(e) => setGradeType(e.target.value)} className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm"><option value="exam">Examen</option><option value="partial">Test</option><option value="proiect">Proiect</option><option value="laborator">Laborator</option></select><input value={gradeWeight} onChange={(e) => setGradeWeight(e.target.value)} placeholder="Pondere %" type="number" min="0" max="100" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /></div><div className="mt-3 flex gap-2"><button type="button" onClick={() => setShowAdd(false)} className="rounded-xl border border-border px-3 py-2 text-xs text-text-secondary">Anulează</button><button type="button" onClick={() => addGrade.mutate()} disabled={!gradeSubject || !gradeValue || addGrade.isPending} className="rounded-xl bg-primary px-3 py-2 text-xs text-white">Adaugă nota</button></div></> : <><div className="grid gap-2 sm:grid-cols-3"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nume materie" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><input value={professor} onChange={(e) => setProfessor(e.target.value)} placeholder="Profesor" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /><input value={credits} onChange={(e) => setCredits(e.target.value)} placeholder="Credite" type="number" className="rounded-xl border border-border bg-transparent px-3 py-2 text-sm" /></div><div className="mt-3 flex gap-2"><button type="button" onClick={() => setShowAdd(false)} className="rounded-xl border border-border px-3 py-2 text-xs text-text-secondary">Anulează</button><button type="button" onClick={() => addSubject.mutate()} disabled={!name.trim() || addSubject.isPending} className="rounded-xl bg-primary px-3 py-2 text-xs text-white">Adaugă</button></div></>}</div>}
          <div className="mb-6">
      ...
      {view === "Taskuri" && <div className="space-y-2">{((data as any)?.academic_work?.tasks ?? []).map((task: any) => <Card key={task.id} className="flex items-center gap-3 p-3"><div className="flex-1"><p className="text-sm font-medium">{task.title}</p><p className="text-xs text-text-secondary">{task.subject_name}{task.due_date ? ` · ${task.due_date}` : ""}</p></div><span className="text-xs text-text-muted">{task.priority}</span></Card>)}</div>}
      {view === "Proiecte" && <div className="space-y-2">{((data as any)?.academic_work?.projects ?? []).map((project: any) => <Card key={project.id} className="flex items-center gap-3 p-3"><div className="flex-1"><p className="text-sm font-medium">{project.name}</p><p className="text-xs text-text-secondary">{project.subject_name}{project.deadline ? ` · ${project.deadline}` : ""}</p></div><span className="text-xs text-text-muted">{project.progress ?? 0}%</span></Card>)}</div>}
