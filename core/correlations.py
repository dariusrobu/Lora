...
async def compute_correlations(pool) -> List[Dict[str, Any]]:
    ...
    snapshot, weekly = await asyncio.gather(
        get_30day_snapshot(pool),
        get_weekly_patterns(pool)
    )
    
    # Count days that actually have some health or productivity data
    days_with_data = sum(1 for d in snapshot if d.get('sleep_hours') is not None or d.get('tasks_completed', 0) > 0)
    
    if days_with_data < 7:
        print(f"Correlation Engine: Not enough actual data (only {days_with_data} days).", flush=True)
        return []
...
async def get_unseen_correlations(pool, computed: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in computed:
        ...
        for pattern in known_patterns:
            ...
            if corr_text.lower() in pattern.lower() or pattern.lower() in corr_text.lower():
                is_known = True
                break
...
async def save_correlation_as_fact(pool, correlation: Dict[str, Any]):
    ...
    await save_memory_fact(pool, "pattern", fact_text, source="inferred", confidence=0.8)
