...
async def calculate_correlations(
    ...
    if len(sleep_days) >= 5 and len(mood_days) >= 5:
        ...
        valid_days = [
            d
            for d in data
            if d["sleep_hours"] is not None and d["mood_score"] is not None
        ]
        if len(valid_days) >= 5:
...
async def generate_suggestions(pool, user_id: int) -> List[str]:
    ...
    async with pool.acquire() as conn:
        ...
        if not last_expense or datetime.now() - last_expense > timedelta(hours=48):
            suggestions.append(
                "Nu ai logat nicio cheltuială în ultimele 48 de ore. Ai omis ceva?"
            )
        ...
        for task in upcoming_tasks:
            suggestions.append(
                f"Mâine expiră task-ul: '{task['title']}'. Ești pe drumul cel bun?"
            )
