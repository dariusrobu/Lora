    ...
    async with pool.acquire() as conn:
        ...
        for row in exams_rows:
            ...
            dt_start = tz.localize(
                datetime.combine(row["exam_date"], time(9, 0))
            )  # Default 09:00
            e.add("dtstart", dt_start)
        ...
        sem_config = await conn.fetchrow(
        ...
        if sem_config:
            ...
            for row in schedule_rows:
                ...
                dt_start = tz.localize(
                    datetime.combine(first_occurrence, row["start_time"])
                )
                dt_end = tz.localize(
                    datetime.combine(first_occurrence, row["end_time"])
                )
