...
async def get_nearby_places(lat: float, lon: float, radius: int = 500, category: str = "shop") -> List[Dict[str, Any]]:
    """
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                for el in elements:
                    ...
                    results.append({
                        "id": el.get("id"),
                        "name": tags.get("name") or tags.get("brand") or "Locație necunoscută",
...
async def is_near_shop(lat: float, lon: float, shop_name: str, radius: int = 300) -> bool:
