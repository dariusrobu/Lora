...
async def analyze_image(client, photo_bytes: bytes, caption: str | None) -> Dict[str, Any]:
...
async def process_vision_result(pool, result: Dict[str, Any], user_profile: Dict[str, Any], client) -> Tuple[str, InlineKeyboardMarkup | None]:
    ...
    if img_type == "receipt":
        ...
        if amount is None:
            return safe_markdown("Am detectat un bon, dar nu am putut citi suma totală clar."), None
        ...
        await set_state(pool, "awaiting_vision_confirmation", "finance", "log_expense", None, extra=extra)
        ...
        try:
            ...
            return safe_markdown("Arată apetisant! Din păcate nu am putut genera o recomandare specifică."), None
        ...
        if not duration:
            return safe_markdown(f"Antrenament detectat ({sport}), dar nu am găsit durata. Încearcă să o introduci manual."), None
        ...
        # 'other' or error
        return safe_markdown(base_reply or "Nu am putut clasifica clar această imagine."), None
...
async def handle_vision_callback(query, pool, callback_data: str):
    ...
    if callback_data == "vision:confirm":
        ...
        if action == "log_expense":
            ...
            await log_finance(pool, "expense", amount, category=category, description=desc)
            await clear_state(pool)
