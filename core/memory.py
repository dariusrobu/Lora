...
async def extract_and_save_facts(pool, client, user_message: str, assistant_reply: str) -> None:
...
Facts should be personal, significant, or related to preferences, patterns, and achievements.
...
[
  {{
    ...
    "fact": "The fact in Romanian (direct, concise, third-person e.g. 'Îi place...', 'Locuiește...')",
...
async def get_context_memory(pool, current_message: str) -> str:
    ...
    try:
        ...
        keywords = [word.lower().rstrip(".,!?") for word in current_message.split() if len(word) >= 3]
        if not keywords:
