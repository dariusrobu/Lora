...
async def handle_travel_intent(
    ...
    if intent == "travel_add":
        ...
        return (
            f"✅ Item-urile {items_joined} au fost adăugate cu succes pe lista *{escape_md(list_name)}*\\.",
            travel_list_keyboard(items_list, list_name),
            last_id,
        )
        ...
        if not items:
            return (
                f"🧳 *Lista de bagaj: {escape_md(list_name)}*\n━━━━━━━━━━━━━━━━━━━━\nLista este goală\\. Drum bun\\!",
                None,
                None,
            )
        ...
        if intent == "travel_check":
            reply = (
                f"🛫 *Drum bun\\!* Nu uita să verifici lista pentru *{escape_md(list_name)}*:\n\n"
                + reply
            )
        ...
        if not item_name:
            return "⚠️ Atenție: Nu ai specificat ce ai pus în bagaj.", None, None

        await mark_item_packed_by_name(pool, list_name, item_name)
        items = await get_travel_items(pool, list_name)
        return (
            f"✅ Item-ul *{escape_md(item_name)}* a fost bifat pe lista *{escape_md(list_name)}*\\.",
            travel_list_keyboard(items, list_name),
            None,
        )
        ...
        if not list_name:
            return "⚠️ Atenție: Nu ai specificat ce listă vrei să resetezi.", None, None

        reset_only = data.get("reset_only", True)
        await clear_travel_list(pool, list_name, reset_only=reset_only)

        action = "resetată" if reset_only else "ștearsă"
        ...
        return (
            f"🗑️ Lista *{escape_md(list_name)}* a fost {action} cu succes\\.",
            markup,
            None,
        )

    return "❌ Eroare: Modulul travel nu recunoaște acest intent.", None, None
...
async def undo_last_action(pool, intent: str, item_id: int) -> Tuple[bool, str]:
    ...
    try:
        if intent == "travel_add":
            ...
            return True, "🗑️ Item-ul de travel a fost șters."
    ...
    if action == "travel":
        ...
        if sub_action == "toggle":
            ...
            rows = await pool.fetch(
                "SELECT is_packed FROM travel_items WHERE id = $1", item_id
            )
            if rows:
            ...
            items = await get_travel_items(pool, list_name)
            await query.edit_message_text(
                f"🗑️ Lista *{escape_md(list_name)}* a fost resetată cu succes\\.",
                parse_mode="MarkdownV2",
                reply_markup=travel_list_keyboard(items, list_name),
            )
