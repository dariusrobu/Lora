...
async def handle_focus_intent(pool, intent: str, data: Dict[str, Any], bot=None) -> Tuple[str, None]:
    ...
    if intent == "focus_start":
        ...
        if row:
            elapsed = int((now - row['created_at'].replace(tzinfo=pytz.utc)).total_seconds() / 60)
            await focus_queries.interrupt_session(pool, session_id, elapsed)
            
            # Try to remove the scheduled job if possible
            try:
                from scheduler.jobs import _global_scheduler
                if _global_scheduler and _global_scheduler.get_job(f"focus_{session_id}"):
        ...
        lines = [f"⏱ *Focus Sessions — ultimele 7 zile*\n"]
        lines.append(f"• Sesiuni completate: *{len(completed)}/{len(sessions)}*")
        lines.append(f"• Timp total focus: *{total_h}h {total_m}min*\n")
        
        for s in sessions[:5]:
            status = "✅" if s['completed'] else ("⚠️" if s.get('interrupted_at') else "⬜")
            task = f" — {escape_md(s['task_description'])}" if s.get('task_description') else ""
