...
async def handle_schedule_intent(pool, intent: str, data: Dict[str, Any], bot=None) -> Tuple[str, None]:
    ...
    if intent == "schedule_today":
        ...
        for c in classes:
            ...
            lines.append(f"{type_str} `{start}–{end}` *{escape_md(c['subject_name'])}*{room}")
        ...
        for day_idx, day_name in days.items():
            ...
            for c in classes:
                ...
                lines.append(f"  {type_icon} `{start}` {escape_md(c['subject_name'])}{room}")
            lines.append("")
