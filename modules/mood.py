...
async def generate_mood_chart(pool, year: int, month: int) -> bytes:
    ...
    ax.axhline(avg_val, color='white', linestyle=':', alpha=0.3, label=f'Medie: {avg_val:.1f}')
...
async def handle_mood_intent(pool, intent: str, data: Dict[str, Any], bot):
    ...
    if intent == "get_mood_chart":
        ...
        if png_bytes is None:
            return "Nu am suficiente date de mood încă. Completează jurnalul câteva zile și reîncearcă. ✍️", None
