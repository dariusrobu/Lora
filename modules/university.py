...
async def handle_university_intent(pool, intent: str, data: Dict[str, Any], bot=None) -> Tuple[str, None]:
    ...
    if intent == "uni_add_subject":
        ...
        details = []
        if credits: details.append(f"{credits} credite")
        if professor: details.append(f"prof. {professor}")
        ...
        if "curs" in query_text:
            return "Lora trackează prezența *doar la seminare*\\. La cursuri nu este necesar\\. 📖", None
        ...
        if not has_seminar:
            return f"Materia *{escape_md(subject['name'])}* nu are seminare în orar, deci nu trackăm prezența\\. 📚", None
        ...
        return f"*{escape_md(subject['name'])}* \\(seminar\\) — {status} înregistrat\\.", None
        ...
        return f"Notă *{grade}* la *{escape_md(subject['name'])}* \\({escape_md(grade_type)}\\) salvată\\.", None
        ...
        return f"Examen *{escape_md(subject['name'])}* pe *{exam_date.strftime('%d %b')}*{loc_str} adăugat\\. 📅", None
        ...
        for w in warnings:
            ...
            lines.append(f"• *{name}*: {w['attended']}/{w['total']} \\({int(w['pct'])}% din minimul de {w['min_attendance_pct']}%\\)")
