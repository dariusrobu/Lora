...
async def handle_workout_intent(
    pool, intent: str, data: dict, bot=None
    ...
    if intent == "workout_log":
        ...
        lines = [
            f"✅ Antrenament salvat: {icon} *{escape_md(sport_name)}*{dur_str}{cal_str}",
        ]
        ...
        return "\n".join(lines), workout_main_keyboard(), workout_id

    elif intent == "workout_list":
        res_text, res_markup = await get_workout_dashboard(pool)
        return res_text, res_markup, None

    elif intent == "workout_week":
        res_text, res_markup = await get_week_summary(pool)
        return res_text, res_markup, None

    elif intent == "workout_stats":
        days = data.get("period_days", 30)
        res_text, res_markup = await get_stats(pool, days)
        return res_text, res_markup, None

    elif intent == "workout_prs":
        res_text, res_markup = await get_personal_records(pool)
        return res_text, res_markup, None
        ...
        return "⚠️ Atenție: Nu ai specificat numele sportului\\.", None, None
        ...
        return "⚠️ Atenție: Nu ai specificat numele exercițiului\\.", None, None

    return "❌ Eroare: Nu am înțeles cererea legată de antrenamente.", None, None
...
async def handle_workout_callback(query, pool, data: str):
    ...
    if data == "workout_main":
            ...
            if i + 1 < len(sports):
                row.append(
                    InlineKeyboardButton(
                        ...
                        callback_data=make_callback_data(
                            "workout", "log", "sport", sports[i + 1]["id"]
                        ),
                    )
                )
            keyboard.append(row)
        keyboard.append(
            [
                InlineKeyboardButton(
                    "⬅️ Înapoi", callback_data=make_callback_data("workout", "main")
                )
            ]
        )
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "❌ Anulează",
                            callback_data=make_callback_data("workout", "main"),
                        )
                    ]
                ]
            ),
        ...
        for w in recent[:10]:
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        label,
                        callback_data=make_callback_data("workout", "delete", w["id"]),
                    )
                ]
            )
        keyboard.append(
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton(
                    "📝 Note",
                    callback_data=make_callback_data(
                        "workout", "edit", "f", "notes", workout_id
                    ),
                )
            ],
            [
                InlineKeyboardButton(
                    "⬅️ Înapoi", callback_data=make_callback_data("workout", "edit")
                )
            ],
        ]
        await query.edit_message_text(
...
async def handle_workout_message(update, pool, state: dict, text: str):
    ...
    try:
        ...
        traceback.print_exc()
        await update.message.reply_text(f"❌ Eroare: Format incorect. {e}")
...
async def undo_last_action(pool, intent: str, item_id: int) -> Tuple[bool, str]:
    ...
    if not workout:
        return False, "❌ Eroare: Antrenamentul nu mai există."

    try:
        await workout_queries.delete_workout(pool, item_id)
        return (
            True,
            f"antrenamentul: {workout['sport_name']} din {workout['workout_date']}",
        )
    except Exception as e:
        return False, str(e)
