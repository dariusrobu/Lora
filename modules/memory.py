...
async def handle_memory_callback(query, pool, data: str):
    ...
    try:
        if data == "memory:delete_last":
            ...
            keyboard = [
                [
                    InlineKeyboardButton("✅ Da, șterge tot", callback_data="memory:clear_all_confirmed"),
                    InlineKeyboardButton("❌ Nu, anulează", callback_data="memory:view_back"),
                ]
            ]
            ...
            await query.edit_message_text("Memoria a fost ștearsă complet.", parse_mode="MarkdownV2")
...
async def handle_memory_intent(pool, intent: str, data: Dict[str, Any]) -> Tuple[str, Any]:
    ...
    if intent == "memory_view":
        ...
        if not memories:
            return "Nu am salvat nicio amintire despre tine încă. 🧠", memory_main_keyboard()
        ...
        if not fact_id and query:
            ...
            if len(results) == 1:
                ...
                return f"Nu am găsit nicio amintire legată de '{escape_md(str(query))}'.", None
                
        if fact_id:
            await memory_queries.delete_fact(pool, int(fact_id))
            return f"✅ Amintirea `#{fact_id}` a fost ștearsă. Am uitat! 💨", None
        
        return "Nu am înțeles ce amintire vrei să șterg. Te rog să menționezi ID-ul (ex: #3).", None
