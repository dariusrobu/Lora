...
async def get_finance_summary(request):
    try:
        ...
        categories = await finance_queries.get_monthly_category_totals(pool, month, year)
        budget_alerts_db = await finance_queries.get_budget_status(pool)
        
        top_categories = [{"category": c['category'], "amount": float(c['total'])} for c in categories[:5]]
        
        budget_alerts = []
        for b in budget_alerts_db:
            if b['current_spent'] > 0 or b['monthly_limit'] > 0: # just returning all that have a limit
...
async def post_memory(request):
    try:
        ...
        if not fact_text:
            return web.json_response({"error": "Missing 'fact' in request body"}, status=400)
