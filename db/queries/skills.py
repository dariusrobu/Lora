...
async def get_skill_by_name(pool, name: str) -> Optional[Dict[str, Any]]:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT * FROM skills WHERE LOWER(name) = LOWER($1)", name.strip()
        )
        return dict(row) if row else None


async def add_skill(
    pool, name: str, category: str = "General", unit: str = "unit"
) -> Dict[str, Any]:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
...
async def log_skill_value(
    pool, skill_id: int, value: float, metric: str = None, log_date: date = None
) -> Dict[str, Any]:
    if log_date is None:
        log_date = date.today()
    async with pool.acquire() as conn:
...
async def get_skill_history(
    pool, skill_id: int, limit: int = 10
) -> List[Dict[str, Any]]:
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
