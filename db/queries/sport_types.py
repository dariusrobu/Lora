...
async def get_all_sports(pool) -> list[dict]:
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM sport_types ORDER BY category ASC, id ASC")
    return [dict(r) for r in rows]

async def get_sports_by_category(pool, category: str) -> list[dict]:
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM sport_types WHERE category = $1 ORDER BY id ASC", category)
    return [dict(r) for r in rows]

async def add_sport(pool, name: str, category: str, has_distance: bool, has_weight: bool, has_reps: bool, icon: str) -> dict:
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            INSERT INTO sport_types (name, category, has_distance, has_weight, has_reps, icon)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *
        """, name, category, has_distance, has_weight, has_reps, icon)
    return dict(row)
...
async def delete_sport(pool, sport_id: int) -> bool:
    async with pool.acquire() as conn:
        ...
        count = await conn.fetchval("SELECT COUNT(*) FROM workouts WHERE sport_id = $1", sport_id)
        if count > 0:
