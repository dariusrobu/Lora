...
async def get_travel_items(
    pool, list_name: str, trip_type: Optional[str] = None
) -> List[Dict[str, Any]]:
    async with pool.acquire() as conn:
...
async def clear_travel_list(pool, list_name: str, reset_only: bool = False):
    async with pool.acquire() as conn:
        if reset_only:
            ...
            await conn.execute(
                "DELETE FROM travel_items WHERE list_name ILIKE $1", list_name
            )
...
async def get_all_travel_lists(pool) -> List[str]:
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT DISTINCT list_name FROM travel_items ORDER BY list_name ASC"
        )
